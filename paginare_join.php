	...
	function extragerePagina($conn,$offset, $nr_rez){
		$sql= "SELECT f.*
				FROM (
    				SELECT t.*, rownum r
    				FROM (
       					SELECT u.username, titlu
       					FROM utilizatori u INNER JOIN petitii p
...
<?php
	...
	$s = ociparse($conn, "SELECT COUNT(*) FROM utilizatori u INNER JOIN petitii p ON u.username=p.username");
    echo '<br />';
    if(ociexecute($s))
    {
        if (ocifetch($s)) 
...
?>
	<table width="600" border="1">
	  <tr>
	    ...
	    <th width="98"> <div align="center">TITLU</div></th>
